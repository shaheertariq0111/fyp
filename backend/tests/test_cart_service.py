...
def test_start_item_customization_resumes_cart_created_state():
    service, carts, _ = build_services()
    started = service.start_item_customization(
        "user",
        "session",
    ...
    before = carts.find_by_cart_id("user", started.data["cart_id"])

    resumed = service.start_item_customization(
        "user",
        "session",
        "configurable",
        2,
    ...
    assert resumed.data["cart_id"] == started.data["cart_id"]
    assert resumed.data["status"] == "cart_created"
    assert resumed.next_action == "set_customization_mode"
    assert carts.find_by_cart_id("user", started.data["cart_id"]) == before
    assert len(carts.data) == 1


def test_start_item_customization_rejects_incompatible_active_cart_state():
    service, carts, _ = build_services()
    started = service.start_item_customization(
        "user",
        "session",
    ...
    service.handle_upsell("user", ready.data["cart_id"], "get_options")
    before = carts.find_by_cart_id("user", ready.data["cart_id"])
    ...
    assert response.error_code == "INVALID_CART_STATE"
    assert carts.find_by_cart_id("user", ready.data["cart_id"]) == before
    assert len(carts.data) == 1


def test_delivery_cannot_start_from_cart_before_pending_order_exists():
    service, carts, orders = build_services()
    started = service.start_item_customization(
        "user",
        "session",
    ...
    response = service.order_service.update_order_flow(
        "user",
        ready.data["cart_id"],
        "set_delivery",
    )
    ...
    assert orders.data == {}
    assert carts.find_by_cart_id("user", ready.data["cart_id"])["status"] == (
