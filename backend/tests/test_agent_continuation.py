    ...
    menu = MemoryMenuRepository(
        items=[
            ...
            },
            {
                ...
                "upsell_group_ids": ["simple-upsells"],
            },
            {
                "product_id": "simple-upsell",
                "name": "Simple Add-on",
                "category": "side",
                "currency": "PKR",
                "available": True,
        ...
        upsells=[
            {
                "upsell_group_id": "simple-upsells",
                "question": "Would you like an add-on?",
                "items": ["simple-upsell"],
            }
        ],
    )
...
def test_item_ready_continuation_exposes_complete_action_and_effect_contract():
    services = build_services()
    start_simple_cart(services)
    ...
    assert continuation.state == "item_ready"
    assert continuation.required_effect is None
    assert continuation.is_outstanding is True
    assert continuation.valid_next_actions == (
    ...
    assert continuation.pending_prompt == (
        "You already have an active cart ready for add-ons or checkout."
    )
    assert "accepted_effects:" in block
    assert "begin_checkout" in block
    assert "create_pending_order_from_cart" in block


def test_upsell_offer_effect_satisfies_item_ready_and_advances_continuation():
    services = build_services()
    started = start_simple_cart(services)
    item_ready = resolve_transactional_continuation(
        services,
        user_id="user",
        agent_session_id="session",
    )
    ...
    offered = services.carts.handle_upsell(
        "user",
        started.data["cart_id"],
        "get_options",
    )
    awaiting = resolve_transactional_continuation(
        services,
        user_id="user",
        agent_session_id="session",
    )

    assert offered.grounding.transactional_effects == ["upsell_offered"]
    assert continuation_satisfied_by(
        item_ready,
        [write_call(offered, "handle_cart_upsell")],
    ) is True
    assert awaiting.state == "awaiting_upsell_decision"
    assert awaiting.is_outstanding is True
    assert awaiting.valid_next_actions == (
    ...
    assert awaiting.accepted_effects == frozenset(
        {"item_added", "cart_progressed", "checkout_started", "cart_cancelled"}
    )
    assert awaiting.pending_prompt == offered.user_message


def test_upsell_skip_satisfies_item_ready_and_makes_cart_ready_outstanding():
    services = build_services()
    started = start_simple_cart(services)
    item_ready = resolve_transactional_continuation(
        services,
    ...
    skipped = services.carts.handle_upsell(
        "user",
        started.data["cart_id"],
        "skip",
    )
    cart_ready = resolve_transactional_continuation(
        services,
        user_id="user",
        agent_session_id="session",
    )
    ...
    assert continuation_satisfied_by(
        item_ready,
        [write_call(skipped, "handle_cart_upsell")],
    ) is True
    assert cart_ready.state == "cart_ready"
    assert cart_ready.required_effect is None
    assert cart_ready.is_outstanding is True
    assert cart_ready.valid_next_actions == (
        "begin_checkout",
        "create_pending_order_from_cart",
        "discard_active_cart",
    )
    assert cart_ready.accepted_effects == frozenset(
        {"checkout_started", "cart_cancelled"}
    )
    assert cart_ready.pending_prompt == "Your existing cart is ready for checkout."


@pytest.mark.parametrize(
    "tool_name",
    ["begin_checkout", "create_pending_order_from_cart"],
)
def test_checkout_tools_satisfy_item_ready_continuation(tool_name):
    services = build_services()
    started = start_simple_cart(services)
    continuation = resolve_transactional_continuation(
        services,
    ...
    pending = services.carts.create_pending_order("user", started.data["cart_id"])
    saved_cart = services.carts.carts.find_by_cart_id("user", started.data["cart_id"])
    ...
    assert saved_cart["status"] == "converted_to_order"
    assert continuation_satisfied_by(
        continuation,
        [write_call(pending, tool_name)],
    ) is True


def test_awaiting_upsell_accepts_every_supported_exit_effect():
    services = build_services()
    started = start_simple_cart(services)
    services.carts.handle_upsell("user", started.data["cart_id"], "get_options")
    continuation = resolve_transactional_continuation(
        services,
        user_id="user",
        agent_session_id="session",
    ...
    for effect in ("item_added", "cart_progressed", "checkout_started", "cart_cancelled"):
        assert continuation_satisfied_by(
            continuation,
            [tool_call(tool_name="cart_write", effects=[effect])],
        ) is True


def test_awaiting_upsell_zero_tool_narration_uses_authoritative_upsell_prompt():
    services = build_services()
    started = start_simple_cart(services)
    offered = services.carts.handle_upsell(
        "user",
    ...
    grounded = ground_agent_response(
        text="Checkout has started. Would you like takeaway?",
        tool_calls=[],
        continuation=continuation,
    )

    assert grounded.text == offered.user_message
    assert grounded.source == "authoritative_continuation"
    assert grounded.rejection_reason == "required_effect_not_satisfied"


def test_zero_tool_item_ready_narration_falls_back_to_authoritative_cart_state():
    services = build_services()
    start_simple_cart(services)
    continuation = resolve_transactional_continuation(
        services,
    ...
    grounded = ground_agent_response(
        text="Checkout has started. Would you like delivery or takeaway?",
        tool_calls=[],
        continuation=continuation,
    )
...
def test_zero_tool_cart_ready_narration_does_not_claim_order_creation():
    services = build_services()
    started = start_simple_cart(services)
    services.carts.handle_upsell("user", started.data["cart_id"], "skip")
    continuation = resolve_transactional_continuation(
        services,
        user_id="user",
        agent_session_id="session",
    ...
    grounded = ground_agent_response(
        text="Your order has been created. Would you like takeaway?",
        tool_calls=[],
        continuation=continuation,
    )

    assert grounded.text == "Your existing cart is ready for checkout."
    assert grounded.source == "authoritative_continuation"
    assert grounded.rejection_reason == "required_effect_not_satisfied"


@pytest.mark.parametrize("effect", ["checkout_started", "cart_cancelled"])
def test_cart_ready_accepts_every_supported_exit_effect(effect):
    services = build_services()
    started = start_simple_cart(services)
    services.carts.handle_upsell("user", started.data["cart_id"], "skip")
    continuation = resolve_transactional_continuation(
