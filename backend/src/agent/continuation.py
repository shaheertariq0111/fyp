...
CONTEXT_INSTRUCTION = (
    "This block is backend truth for the current customer and is already the "
    "fresh authoritative backend state for this turn. Conversation history is "
    "never proof that any of it changed. Interpret the customer's own words "
    "freely. When selection_tool, offered_options, fixed_arguments, and "
    "selected_option_argument are present and the customer's current message "
    "semantically identifies exactly one offered option, call selection_tool "
    "directly using the fixed arguments plus that option's authoritative ID. "
    "Do not call get_active_cart merely to rediscover the same pending choice. "
